  ...
  constructor(attrs?, options?) {
    ...
    this.once('change', () => {
      this.on('change:currentMin', (model, value) => {
        this.set({ handleMinLeft: this.getLeftFromYear(value) })
      });
      this.on('change:currentMax', (model, value) => {
        this.set({ handleMaxLeft: this.getLeftFromYear(value) })
      });
      this.on('change:handleMinLeft', (model, value) => {
        this.set({ currentMin: this.getYearFromLeft(value) })
      });
      this.on('change:handleMaxLeft', (model, value) => {
        this.set({ currentMax: this.getYearFromLeft(value) })
      });
    });
  }
